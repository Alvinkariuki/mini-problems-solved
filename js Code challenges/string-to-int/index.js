// ***THE SNIPPET BELOW IS NOT MY CODE***
function convertStringToInteger(str){
  /**
   * map string numbers to integers
   * 
   * map string numbers with -ty and multiply by 10
   * 
   * add all values to array, sort from largest to smallest
   * 
   * Add values based on sort
   */
   var words = {
    "zero":0, "one":1, "two":2, "three":3, "four":4, "five":5, "six":6, "seven":7, "eight":8, "nine":9, 
    "ten":10, "eleven":11, "twelve":12, "thirteen":13, "fourteen":14, "fifteen":15, "sixteen":16, 
    "seventeen":17, "eighteen":18, "nineteen":19, "twenty":20, "thirty":30, "forty":40, "fifty":50, 
    "sixty":60, "seventy":70, "eighty":80, "ninety":90
  };
  var mult = { "hundred":100, "thousand":1000, "million": 1000000 };

    return str.split(/ |-/).reduce(function(value, word) {
      if (words[word]) value += words[word];
      if (mult[word]) value += mult[word] * (value % mult[word]) - (value % mult[word]);
      return value;
    },0);
}

console.log(convertStringToInteger(["seven fifty thousand and fifty"]));
